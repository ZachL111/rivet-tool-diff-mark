...
`rivet-tool-diff-mark` is centered on cli tools work in Python. The technical target is: Package a Python local lab for diff analysis with fixture event logs, golden state snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 177. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 3.
